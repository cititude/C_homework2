...
void Boa::automove()
{
    ...
    if(board->get_impedenceid(get_next())>=2||p>0.8||board->get_snakeid(get_next())>id)
    {
        int dir=1;
        ...
        if(board->get_impedenceid(up)>=2||board->get_snakeid(up)>id)a1=0;
        if(board->get_impedenceid(right)>=2||board->get_snakeid(right)>id)a2=0;
        if(board->get_impedenceid(down)>=2||board->get_snakeid(down)>id)a3=0;
        if(board->get_impedenceid(left)>=2||board->get_snakeid(left)>id)a4=0;
        int sum=a1+a2+a3+a4;
